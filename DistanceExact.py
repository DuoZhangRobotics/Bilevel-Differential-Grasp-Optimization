...
def find_lib_path():
    ...
    for p in os.listdir(path):
        ...
        if os.path.exists(full_path+'/libDistanceExact.so'):
            return full_path+'/libDistanceExact.so'
        if os.path.exists(full_path+'/libDistanceExact.a'):
            return full_path+'/libDistanceExact.a'
    assert False
...
class DistanceExact(torch.autograd.Function):
    ...
    def grad_check(hand,mesh_paths):
        configurations=torch.randn(len(mesh_paths),hand.nr_dof()+hand.extrinsic_size)
...
if __name__=='__main__':
    mesh_paths=['cube/sphere_unit.obj']
