#ifndef MARCHING_CUBE_2D_VOLUME_H
#define MARCHING_CUBE_2D_VOLUME_H
...
template <typename T>
class MarchingCube2DVolume
{
public:
  typedef typename ScalarUtil<T>::ScalarVec3 PT3;
  typedef typename ObjMeshTraits<T>::Type ObjMeshType;
  MarchingCube2DVolume(ObjMeshType& mesh):_vss(mesh.getV()),_iss(mesh.getI()) {
    mesh.setDim(3);
  }
  virtual ~MarchingCube2DVolume() {}
  void solve(const Grid<T,T>& grd,const T& contour=0.0f) {
    _vss.clear();
    _iss.clear();

    ASSERT(grd.getDim() == 2);
    Grid<Vec3i,T> vertIndex;
    vertIndex.makeSameGeometry(grd);
    vertIndex.init(Vec3i::Constant(-1));
    ...
    for(sizeType xx=0; xx<nrPoint.x(); xx++)
      for(sizeType yy=0; yy<nrPoint.y(); yy++) {
        ...
        if(grd.get(Vec3i(xx,yy,0))-contour<=0) {
          vertIndex.get(Vec3i(xx,yy,0))[2]=_vss.size();
          _vss.push_back(grd.getPt(Vec3i(xx,yy,0)));
        }
      }
    ...
    for(int xx=0; xx<nrPoint[0]-1; ++xx)
      for(int yy=0; yy<nrPoint[1]-1; ++yy) {
        ...
        sizeType nrTri=nrTriangles2DV[(sizeType)marker];
        for(int i=0,j=0; i<nrTri; ++i,j+=3) {
          Vec3i tmp=Vec3i::Zero();
          tmp[0]=chooseVert(vertIndex,xx,yy,idTriangles2DV[marker*12+j]);
          tmp[1]=chooseVert(vertIndex,xx,yy,idTriangles2DV[marker*12+j+1]);
          tmp[2]=chooseVert(vertIndex,xx,yy,idTriangles2DV[marker*12+j+2]);
          _iss.push_back(tmp);
        }
      }
  }
  static PT3 genPoint(const PT3& a,const PT3& b,const T& ia,const T& ib) {
    ...
    return PT3(tmp.x(),tmp.y(),tmp.z());
  }
  sizeType chooseVert(const Grid<Vec3i,T>& grd,const sizeType& xx,const sizeType& yy,const sizeType& edge) {
    sizeType ret=-1;
    switch(edge) {
    //edge
    case 0:
    ...
    //vertex
    case 4:
      ret=grd.get(Vec3i(xx,yy,0)).z();
      break;
    case 5:
      ret=grd.get(Vec3i(xx+1,yy,0)).z();
      break;
    case 6:
      ret=grd.get(Vec3i(xx+1,yy+1,0)).z();
      break;
    case 7:
      ret=grd.get(Vec3i(xx,yy+1,0)).z();
      break;
    }
    ASSERT(ret >= 0);
    return ret;
