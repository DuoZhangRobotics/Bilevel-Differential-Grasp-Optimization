...
public:
  ParticleSetTpl():Serializable(typeid(ParticleSetTpl<P_TYPE>).name()) {}
...
struct ParticleBase : public Serializable {
  ...
  ParticleBase():Serializable(typeid(ParticleBase<T>).name()),_pos(0.0f,0.0f,0.0f) {}
...
struct Particle : public ParticleBase<T> {
  ...
  Particle():ParticleBase<T>(typeid(Particle<T>).name()),_vel(0.0f,0.0f,0.0f) {}
  template<typename OTHER>
...
struct ParticleN : public Particle<T> {
  ...
  ParticleN():Particle<T>(typeid(ParticleN<T>).name()),_normal(0.0f,0.0f,0.0f) {}
  template<typename OTHER>
...
struct ParticleSPH : public ParticleN<T> {
  ...
  ParticleSPH():ParticleN<T>(typeid(ParticleSPH<T>).name()) {
    _oldPos.setZero();
    _oldVel.setZero();
    _density=0.0f;
    _pressure=0.0f;
