...
SphericalBoxGeomCell::SphericalBoxGeomCell()
{
  setType(typeid(SphericalBoxGeomCell).name());
}
SphericalBoxGeomCell::SphericalBoxGeomCell(const Mat4& T,sizeType dim,const Vec4& ext)
...
BBox<scalar> SphericalBoxGeomCell::getBBInner() const
{
  return BoxGeomCell::getBBInner().enlarge(_rad,_dim);
}
bool SphericalBoxGeomCell::distInner(const Vec3& pt,Vec3& n) const
{
  return closestInner(pt,n,NULL);
}
bool SphericalBoxGeomCell::closestInner(const Vec3& pt,Vec3& n,Vec3* normal) const
{
  Vec3 normalTmp;
...
scalar SphericalBoxGeomCell::rayQueryInner(const Vec3& x0,const Vec3& dir) const
{
