...
ObjMeshGeomCell::ObjMeshGeomCell(const Mat4& trans,const ObjMesh& mesh,scalar depth,bool buildBVHAsWell,bool bottomUp)
...
BBox<scalar> ObjMeshGeomCell::getBBInner() const
{
  if(!_grid.data().empty())
    return _grid.getBB();
  if(_bvh.empty())
    return BBox<scalar>(Vec3::Zero(),Vec3::Zero());
