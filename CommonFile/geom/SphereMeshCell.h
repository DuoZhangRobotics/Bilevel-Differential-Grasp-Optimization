...
struct ALIGN_16 TwoSphereMeshCell : public ObjMeshGeomCell {
  ...
  TwoSphereMeshCell();
  TwoSphereMeshCell(sizeType dim,const Vec3& ctr1,const Vec3& ctr2,scalar rad1,scalar rad2);
...
struct ALIGN_16 ThreeSphereMeshCell : public ObjMeshGeomCell {
  ...
  using Serializable::write;
  ThreeSphereMeshCell();
  ThreeSphereMeshCell(sizeType dim,const Vec3& ctr1,const Vec3& ctr2,const Vec3 ctr3,scalar rad1,scalar rad2,scalar rad3);
  virtual bool read(std::istream& is,IOData* dat);
  virtual bool write(std::ostream& os,IOData* dat) const;
  std::shared_ptr<SerializableBase> copy() const;
  scalarD rad1() const;
