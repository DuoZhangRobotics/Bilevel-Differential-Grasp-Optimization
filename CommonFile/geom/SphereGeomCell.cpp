...
SphereGeomCell::SphereGeomCell(const Mat4& T,sizeType dim,scalar rad,scalar depth)
  :StaticGeomCell(T,dim,typeid(SphereGeomCell).name()),_rad(rad),_depth(depth)
{
  build(false);
}
BBox<scalar> SphereGeomCell::getBB(bool ref) const
{
  if(ref)
    return getBBInner();
  else {
...
bool SphereGeomCell::dist(const Vec3& pt,Vec3& n) const
{
...
bool SphereGeomCell::closest(const Vec3& pt,Vec3& n,Vec3* normal) const
{
