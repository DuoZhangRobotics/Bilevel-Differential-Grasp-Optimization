...
//Geom
StaticGeomCell::StaticGeomCell(const std::string& type):Serializable(type) {}
StaticGeomCell::StaticGeomCell(const Mat4& T,sizeType dim,const std::string& type)
...
BBox<scalar> StaticGeomCell::getBB(bool ref) const
{
  Vec3 pt;
...
bool StaticGeomCell::closest(const Vec3& pt,Vec3& n,Vec3* normal) const
{
  Vec3 pt0=transformHomo<scalar>(_invT,pt);
...
BBox<scalar> StaticGeomCell::getBBInner() const
{
  FUNCTION_NOT_IMPLEMENTED
  return BBox<scalar>();
}
bool StaticGeomCell::distInner(const Vec3& pt,Vec3& n) const
{
  return closestInner(pt,n);
}
bool StaticGeomCell::closestInner(const Vec3& pt,Vec3& n,Vec3* normal) const
{
  FUNCTION_NOT_IMPLEMENTED
  return false;
}
...
PRJ_END
StaticGeom::StaticGeom():Serializable(typeid(StaticGeom).name())
{
  _bvh.reset(new std::vector<Node<std::shared_ptr<StaticGeomCell>,BBox<scalar> > >);
}
StaticGeom::StaticGeom(sizeType dim):Serializable(typeid(StaticGeom).name()),_dim(dim)
{
  _bvh.reset(new std::vector<Node<std::shared_ptr<StaticGeomCell>,BBox<scalar> > >);
}
