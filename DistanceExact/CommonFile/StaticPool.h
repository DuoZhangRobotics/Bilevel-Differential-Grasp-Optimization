...
class ALIGN_16 StaticStack
{
public:
  EIGEN_DEVICE_FUNC StaticStack():_ptr(0) {}
  bool read(std::istream& is,IOData* dat)
  {
    for(sizeType i=0; i<NR+1; i++)
      readBinaryData(_pool[i],is,dat);
    readBinaryData(_ptr,is);
    return is.good();
  }
  bool write(std::ostream& os,IOData* dat) const
  {
    for(sizeType i=0; i<NR+1; i++)
      writeBinaryData(_pool[i],os,dat);
    writeBinaryData(_ptr,os);
    return os.good();
  }
  std::shared_ptr<SerializableBase> copy() const
  {
    return std::shared_ptr<SerializableBase>(new StaticStack<T,NR>);
  }
  EIGEN_DEVICE_FUNC const T& operator[](sizeType id) const {
    return _pool[id];
  }
  EIGEN_DEVICE_FUNC T& operator[](sizeType id) {
    return _pool[id];
  }
  ...
  EIGEN_DEVICE_FUNC sizeType size() const {
    return _ptr;
  }
  EIGEN_DEVICE_FUNC void resize(sizeType sz) {
    _ptr=sz;
  }
  EIGEN_DEVICE_FUNC bool push(T p) {
    if(_ptr == NR)
      return false;
    else {
      _pool[_ptr++]=p;
  ...
  EIGEN_DEVICE_FUNC T pop() {
    ASSERT(!empty());
    return _pool[--_ptr];
  }
protected:
  ALIGN_16 T _pool[NR+1];
  ALIGN_16 sizeType _ptr;
};
template <typename T>
class StaticPool
{
