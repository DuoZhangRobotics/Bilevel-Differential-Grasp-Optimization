...
template <typename T>
LogBarrierObjEnergy<T>::LogBarrierObjEnergy(const GraspPlanner<T>& planner,const GraspQualityMetric<T>& obj,T d0,T mu,const bool& useGJK)
  :ArticulatedObjective<T>(planner,obj),_useGJK(useGJK),_updateCache(false),_d0(d0),_d1(d0/2),_mu(mu) {}
template <typename T>
int LogBarrierObjEnergy<T>::operator()(const Vec&,const PBDArticulatedGradientInfo<T>& info,sizeType,ParallelMatrix<T>& e,ParallelMatrix<Mat3XT>* g,ParallelMatrix<Mat12XT>* h,Vec*,MatT*)
{
  ...
  if(std::is_same<T,mpfr::mpreal>::value) {
    for(sizeType i=0; i<(sizeType)pairs.size(); i++)
      addTerm(valid,pairs[i],feats[i],info,e,g,h);
  } else {
    OMP_PARALLEL_FOR_
    for(sizeType i=0; i<(sizeType)pairs.size(); i++)
      addTerm(valid,pairs[i],feats[i],info,e,g,h);
...
template <typename T>
void LogBarrierObjEnergy<T>::setUpdateCache(bool update)
{
  _updateCache=update;
}
template <typename T>
std::string LogBarrierObjEnergy<T>::name() const
{
  return "LogBarrierObjEnergy(d0="+std::to_string(_d0)+",mu="+std::to_string(_mu)+")";
}
template <typename T>
void LogBarrierObjEnergy<T>::addTerm(bool& valid,const Vec2i& termId,Vec2i& feat,const PBDArticulatedGradientInfo<T>& info,ParallelMatrix<T>& e,ParallelMatrix<Mat3XT>* g,ParallelMatrix<Mat12XT>* h) const
{
  const std::vector<Node<sizeType,BBox<scalar>>>& bvhObj=_obj.getBVH();
  if(!valid)
    return;
  ...
  if(dist<=0)
    ...
    T D,DD,E=MultiPrecisionSeparatingPlane<T>::clog(dist-_d1,g?&D:NULL,h?&DD:NULL,_d0-_d1,_mu);
    if(!std::isfinite(E)) {
      valid=false;
      return;
    } else if(E==0)
