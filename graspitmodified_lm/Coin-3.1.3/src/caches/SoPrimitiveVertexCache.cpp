...
namespace {
  ...
  SbBool SoGLLazyElement_enabled(SoState * state) {
    if (state->isElementEnabled(SoLazyElement::getClassStackIndex())) {
      return SoLazyElement::getInstance(state)->getTypeId().isDerivedFrom(SoGLLazyElement::getClassTypeId());
    }
    return FALSE;
  }
};
...
// temporary functions for fixing a bug. This is done properly in Coin-4

SbBool
SoPrimitiveVertexCacheUtil::testValid(SoPrimitiveVertexCache * cache, SoState * state)
{
  if (SoGLLazyElement_enabled(state)) {
    if (!SoGLLazyElement::preCacheCall(state , &PRIVATE(cache)->prestate)) return FALSE;
  }
  return cache->isValid(state);
}

void
SoPrimitiveVertexCacheUtil::close(SoPrimitiveVertexCache * cache, SoState * state)
{
  if (SoGLLazyElement_enabled(state)) {
