...
IVmgr::transRot(DraggerInfo *dInfo)
{
  ...
  if (translating) {
    ...
    SbVec3f axis;
    float angle;
    SoSFRotation tmpR=myCenterball->rotation;
    tmpR.getValue(axis,angle);
    SbRotation myCenterBallR(axis,angle);

    desiredRotation.x() = myCenterBallR.getValue()[0];
    desiredRotation.y() = myCenterBallR.getValue()[1];
    desiredRotation.z() = myCenterBallR.getValue()[2];
    desiredRotation.w() = myCenterBallR.getValue()[3];
    SbVec3fTovec3(dInfo->draggerTran->scaleFactor.getValue(), scale);
