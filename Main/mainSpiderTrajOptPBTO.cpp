...
void trajOpt(int argc,char** argv,bool debugDynamics)
{
  ...
  optStage1.formulation(TrajOpt<T,3>::STATIC_PBTO);
  ...
  optStage1.debug(10,0,[&](const std::string& str) {
    return debugDynamics ? (str=="PBDStaticSequence" || str=="PBDDynamicsSequence" || endsWith(str,"PBTOForceSequencePose")) : false;
  });
  ...
  optStage2.formulation(TrajOpt<T,3>::PBTO);
  ...
  optStage2.constrainRootVelocity(Coli::LinSpaced(197,3,199),Vec3d(0.5,0,0),1);
