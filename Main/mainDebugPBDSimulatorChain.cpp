...
void debugGradientInfoChain(const std::string& path,T dt,T dtw,PBD_SIMULATOR_MODE mode,bool floor=true,sizeType root=Joint::BALL_JOINT,sizeType nrLink=10,sizeType GD=1)
{
  DECL_MAP_TYPES_T
  ...
  sim1.setState(Vec::Zero(N),Vec::Zero(N));
...
int main()
{
  ...
  RandEngine::seed(0);
  debugGradientInfoChain("chain-PBD-PGM-0.025",0.025,0.025,NPBD_PGM);
#endif
  return 0;
}
