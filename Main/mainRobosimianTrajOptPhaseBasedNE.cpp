...
void trajOpt(int argc,char** argv,bool debugDynamics)
{
  ...
  for(sizeType i=0; i<body->nrJ(); i++) {
    ...
    if(J._typeJoint==Joint::TRANS_3D)
      pose.segment<3>(J._offDOF)=Vec3d(10,0,0);
    if(J._typeJoint==Joint::TRANS_2D)
      pose.segment<2>(J._offDOF)=Vec2d(10,0);
  ...
  optStage1.debug(10,0,[&](const std::string& str) {
    return debugDynamics ? (str=="NEStaticSequence" || str=="NEDynamicsSequence") : false;
  });
  optStage1.setInitPose(pose.template cast<T>());
  optStage1.solve();
  optStage1.writeVTK("solutionPhaseBasedStage1");
  //Stage II
#ifdef STAGE_II
  ...
  optStage2.constrainRootVelocity(Coli::LinSpaced(197,3,199),Vec3d(0.3,0,0));
...
#endif
  finalStage.writeVTK("solutionPhaseBasedNE");
}
